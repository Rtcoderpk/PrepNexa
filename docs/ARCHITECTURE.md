...
3. `InterviewChat` hydrates existing questions or calls `/api/interview/opening`.
   - Opening generated via `OllamaProvider.chat` (`buildInterviewerSystemPrompt`).
...
5. On send, the client:
   ...
   - `/api/interview/respond` persists the answer + telemetry, runs a semantic
     scoring pass concurrently (embedding cosine blended with LLM judgment), and
     persists the per-question score.
   - The LLM provider generates the next question (adaptive difficulty, no repeats).
...
- **CV is free** (browser-side) → the dominant cost is Ollama inference and
  Whisper transcription. A **feedback queue** seam (Redis list in
  `lib/feedback-queue.ts`) defers slow LLM report generation off the request
  path when `FEEDBACK_QUEUE=redis`; a cron drains it via `/api/feedback-worker`.
...
- Secure headers: CSP (script-src allows `unsafe-eval` for the MediaPipe WASM
  bundle, plus `cdn.jsdelivr.net` / `storage.googleapis.com` for model assets),
  HSTS, X-Frame-Options, Referrer-Policy, Permissions-Policy (camera/mic
  restricted to `(self)`).
